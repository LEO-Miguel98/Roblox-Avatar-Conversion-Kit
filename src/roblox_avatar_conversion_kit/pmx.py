...
def _geometry_only_textured_materials(features: FeaturePlan | None) -> bool:
    if not features:
        return False
    if any(getattr(item, 'accessory_type', '') == 'GeometryOnly' for item in features.layered_clothing):
        return True
    return any(d.name in {'Shark tail', 'Heart hair / cowlick'} for d in features.dynamics)
...
def write_pmx(output:Path,*,model_name:str,mesh:ObjMesh,materials:dict[str,Material],bones:list[Bone],group_to_bone:dict[str,str],smooth_weights=True,features:FeaturePlan|None=None,accessory_physics=True):
    ...
    spring={d.group:d for d in (features.dynamics if features and accessory_physics else ()) if d.physics_mode=='spring'}; planar=_planar_uv_flip_groups(mesh); neutralize_textured=_geometry_only_textured_materials(features)
    textures=[]
    ...
    blob=bytearray(b'PMX ');blob+=struct.pack('<fB',2.0,8)+bytes([0,0,4,4,4,4,4,4]);comment='Generated by Roblox Avatar Conversion Kit v0.3.10. Skin weights and accessory physics are reconstructed and approximate.';blob+=_text(model_name)+_text(model_name)+_text(comment)+_text(comment)
    ...
    for num,mname in enumerate(material_order):
        m=materials.get(mname or '') or Material(mname or f'Material{num+1}'); diffuse=_pmx_diffuse(m,neutralize_textured=neutralize_textured);blob+=_text(m.name)+_text(m.name)+_vec4((*diffuse,m.alpha))+_vec3(m.ks)+struct.pack('<f',max(0,m.ns))+_vec3(tuple(c*.35 for c in diffuse))+struct.pack('<B',0x0F)+_vec4((0,0,0,1))+struct.pack('<f',0)+struct.pack('<iiBBB',texidx.get(m.map_kd,-1),-1,0,1,0)+_text('')+struct.pack('<i',len(indices_by_material[mname]))
