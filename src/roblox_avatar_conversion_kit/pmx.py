...
def _planar_uv_flip_groups(mesh: ObjMesh, threshold: float = 0.025) -> set[str]:
    """Detect extremely thin accessory billboards that are viewed from the opposite side in MMD.

    Roblox accessory exports can contain double-sided textured planes such as signs and speech bubbles.
    The global Roblox->MMD handedness conversion reverses which side is viewed. Their V origin still needs
    the normal OBJ->PMX conversion, while U must additionally be mirrored to keep text/art readable.
    Only very thin Handle* groups qualify so ordinary hair, clothing, swords and body geometry are untouched.
    """
    result: set[str] = set()
...
def write_pmx(
    ...
    *,
    ...
    planar_uv_flip_groups = _planar_uv_flip_groups(mesh)
    ...
    for face in mesh.faces:
        ...
        for vertex_index, uv_index, normal_index in face.corners:
            ...
            if key not in vertex_map:
                ...
                uv = _mmd_uv(
                    mesh.uvs[uv_index],
                    flip_u=face.group in planar_uv_flip_groups,
                ) if uv_index is not None and 0 <= uv_index < len(mesh.uvs) else (0.0, 0.0)
    ...
    comment = "Generated by Roblox Avatar Conversion Kit v0.3.7. Skin weights and accessory physics are reconstructed and approximate."
    ...
    return {
        ...
        "planar_uv_flip_groups": len(planar_uv_flip_groups),
