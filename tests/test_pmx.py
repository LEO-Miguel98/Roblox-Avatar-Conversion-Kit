...
class PmxTests(unittest.TestCase):
    ...
    def test_writes_pmx_with_gaze_ik_and_accessory_physics(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(stats["ik_bones"], 4)
            self.assertEqual(stats["toe_ik_bones"], 2)
            ...
            self.assertIn("全ての親".encode("utf-16-le"), data)
            self.assertIn("グルーブ".encode("utf-16-le"), data)
            self.assertIn("左つま先ＩＫ".encode("utf-16-le"), data)
